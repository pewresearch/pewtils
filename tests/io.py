...
class IOTests(unittest.TestCase):
    ...
    def test_filehandler_get_key_hash(self):
        from pewtils.io import FileHandler
        h = FileHandler("tests/files", use_s3=False)
        self.assertEqual(h.get_key_hash("temp"), "c51bf90ccb22befa316b7a561fe9d5fd9650180b14421fc6d71bcd57")
    ...
    def tearDown(self):
        ...
        import os
        try: os.unlink("tests/files/temp/temp.txt")
        except OSError: pass
        for format in ["pkl", "csv", "tab", "txt", "xlsx", "xls", "dta", "json"]:
            try: os.unlink("tests/files/temp.{}".format(format))
            except OSError: pass
        try: os.rmdir("tests/files/temp")
        except OSError: pass